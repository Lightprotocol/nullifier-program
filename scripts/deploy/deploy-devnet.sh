...
libraries=("light_nullifier_program")
...
create_buffer_account() {
    ...
    while (( attempt <= max_retries )); do
        ...
        if solana program deploy target/deploy/"$program_name".so --program-id "target/deploy/$program_name-keypair.json" --buffer "$BUFFER_KEYPAIR_PATH-$program_name_keypair-keypair.json"; then
